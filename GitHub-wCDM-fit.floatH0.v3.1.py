...
print(
    f"H0Pan    = {H0Pan_best:.2f}"
    f" +/- {sigH0Pan:.2f}"
)

print(
    f"H0DES    = {H0DES_best:.2f}"
    f" +/- {sigH0DES:.2f}"
)
...
def mu_kolb(params):
    """
    Kolb distance modulus with independently fitted H0 values
    for Pantheon+ and DES.
    """
    ...
    # Use the same wCDM integral routine,
    # evaluated exactly at the Kolb point.
...
def kolb_whitened_resid(params):
    residual = mu_data - mu_kolb(params)
...
# Initial values can simply use the wCDM best-fit H0 values
p0_kolb = np.array([
    H0Pan_best,
    H0DES_best
])
...
if not res_kolb.success:
    raise RuntimeError(
        "Kolb fit did not converge: " + res_kolb.message
    )


H0Pan_kolb = float(res_kolb.x[0])
H0DES_kolb = float(res_kolb.x[1])

chi2_kolb = float(
    np.dot(res_kolb.fun, res_kolb.fun)
)
...
BIC_kolb = (
    chi2_kolb
    + n_par_kolb * np.log(n_data)
)

print()
print("Kolb point")
print("----------")
print(f"H0Pan     = {H0Pan_kolb:.2f}")
print(f"H0DES     = {H0DES_kolb:.2f}")
print(f"OmegaM    = {1.0 - OMEGADE_KOLB:.3f}")
print(f"w         = {W_KOLB:.3f}")
